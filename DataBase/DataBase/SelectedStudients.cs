...
namespace DataBase
{
    public partial class SelectedStudients : Form
    {
        public event Action<List<Control>, Boolean> SelectedStd;
        private DatabaseEntities studients;
        private DataGridView data;
        public SelectedStudients(DataGridView data, DatabaseEntities studients)
        {
            InitializeComponent();
            this.Load += SelectedStudients_Load;
            this.data = data;
            this.studients = studients;
            //заполнение comboboxов
            var collectionSport = studients.Спортзал
               .ToList();
            ...
            var collectionGym = studients.Качалка
               .ToList();
            foreach (var item in collectionGym)
            {
                comboBox2.Items.Add(item.Секция);
            }
            //его макс. вместимость
            comboBox1.DropDownHeight = 200;
            comboBox2.DropDownHeight = 200;
        }
        //выбрать студентов из датагрида
        private void SelectedStudients_Load(object sender, EventArgs e)
        {
            ...
            textBox3.Text = mass[3].Value.ToString();
            comboBox1.Text = mass[5].Value.ToString();
            comboBox2.Text = mass[4].Value.ToString();
            dateTimePicker1.Value = DateTime.Parse(mass[6].Value.ToString());
        }
        ...
        private void PictureBox1_Click(object sender, EventArgs e)
        {
            ...
            {
                SelectedStd?.Invoke(this.Controls.Cast<Control>().Where(c => c is TextBox || c is DateTimePicker || c is ComboBox).ToList(), true);
                Close();
            }
        }
